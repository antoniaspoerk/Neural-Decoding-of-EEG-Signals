...
for iterX = 1:iterationX
    ...
    % Reshape into conditions x trials x electrodes x timepoints
    % (raw trials, no pseudo-averaging in this part)
    clear pseudoData
    for condIndex = 1:conditionM
        conditionMatrix = newdataCell{condIndex, 1};
        rawTrialNumber = size(conditionMatrix, 1);
        pseudoData(condIndex,:,:,:) = conditionMatrix(randperm(rawTrialNumber), :, :);
    end

    cv = cvpartition(minC,"kfold",n_folds);
...
for permX = 1:n_perm
    ...
    for iterX = 1:iterationX
        ...
        trueLabels = [ones(minC,1); 2*ones(minC,1)];
...
title('Animal vs Non-animal decoding with single trials - Subject 0');
legend({'95% CI permutation', 'Permutation mean', 'Decoding accuracy'});

%% Save results
save('DA_part3.mat', 'DA');
save('DA_sh_part3.mat', 'DA_perm');
